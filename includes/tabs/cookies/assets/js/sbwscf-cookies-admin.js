...
document.addEventListener('DOMContentLoaded', () => {
	...
	const editorSettings = window.sbwscfTrackingEditorSettings
		? window.sbwscfTrackingEditorSettings
		: {}

	let nextIndex =
		Array.from(tableBody.rows).reduce((max, row) => {
	...
	const addRow = (index) => {
		...
		row.innerHTML = `
                        ...
                        <td><button type="button" class="button sbwscf-remove-script">${__(
							'Delete',
							'smile-basic-web'
						)}</button></td>
                `
	...
	tableBody.addEventListener('click', (event) => {
		...
		if (
			...
			window.wp &&
			window.wp.editor &&
			'function' === typeof window.wp.editor.remove
		) {
